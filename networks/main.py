...
def build_GANs(net_name):
    ...
    if net_name == 'cardio_mlp':
        net = MLP_Classifier(x_dim=21, h_dims=[32, 16], rep_dim=8,out_dim=num_clt, bias=False)
    
    if net_name == 'satellite_mlp':
        net = MLP_Classifier(x_dim=36, h_dims=[32, 16], rep_dim=8,out_dim=num_clt, bias=False)
    
    if net_name == 'satimage-2_mlp':
        net = MLP_Classifier(x_dim=36, h_dims=[32, 16], rep_dim=8,out_dim=num_clt, bias=False)
    
    if net_name == 'shuttle_mlp':
        net = MLP_Classifier(x_dim=9, h_dims=[32, 16], rep_dim=8,out_dim=num_clt, bias=False)
