...
# class FashionMNIST_Similiar(BaseNet): ## @b_trainer.py和@c_trainer.py分别对应以下两个阶段的
    # def __init__(self, x_dim=64, h_dims=[128,64], rep_dim=64,out_dim = 2, bias=False):
    #     super().__init__()
    ...
    #     else:
    #         return self.sigmoid(out), latent
class FashionMNIST_Similiar(BaseNet):
    def __init__(self, x_dim=64, h_dims=[128,64], rep_dim=64, out_dim = 2, bias=False):
        super().__init__()
        ...
        self.code = Linear_BN_leakyReLU(h_dims[-1], self.rep_dim, bias=bias)
        self.fc  = nn.Linear(self.rep_dim*2,self.out_dim,bias=bias)
        self.sigmoid = nn.Sigmoid()

    def forward(self, x,x2, get_latent=False): # x2,
        x = x.view(int(x.size(0)), -1) # torch.Size([89, 64])
        # x2=x.clone()
        # x2 = x2.view(-1, int(x2.size(0)))# torch.Size([64, 1])
        # x2=x2.repeat(int(x.size(0)),1)
        # print("@@@@@@@@@@@@@@@@",x2.size())
        for layer in self.hidden:
            x = layer(x)
        ...
        out = self.fc(latent)
        if get_latent==False:
            #return self.sigmoid(out):  # for BCEloss 鼓楼医院
            return out
        else:
            return self.sigmoid(out), latent
