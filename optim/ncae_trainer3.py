...
class NCAETrainer(BaseTrainer):
    ...
    def __init__(self, optimizer_name: str = 'adam', lr: float = 0.001 ,gan_lr: float = 0.0002, std:float=1.0,idt:float=1.0,lamdba:float=0.1, n_epochs: int = 150, lr_milestones: tuple = (),spl:float=0.7,spm:float=0.7,
    ...
    def train(self, dataset: BaseADDataset, net: BaseNet,d_l, d_s,d_g,g,d_k):
        ...
        for epoch in range(self.n_epochs):
            ...
            for _s, data in enumerate(train_loader):
                ...
                ##  设置初始的inputs_old_max，也要设置轮回的inputs_old_max，不过可能不是inputs_old_max，而是latent_old_max
                # if n_batches==0:
                ...
                output_min = similar(latent_zhong.detach(),x_mu.detach()) # 后面改进similar，这是跟mu的
                ...
                output_max = similar(latent_zhong.detach(),x_mumax.detach()) # 后面改进similar，这是跟mumax的
                ...
                output_min = similar(noise_mu.detach(),x_mu.detach()) # 后面改进similar，这是跟mu的
                output_min = output_min.squeeze()
                ...
                mumax=self.mu_max.repeat(int(latent_min.size(0)),1)
                output_max = similar(noise_mu.detach(),mumax.detach()) # 后面改进similar，这是跟mumax的
                output_max = output_max.squeeze()
                errsimilar_max = 0.1-criterion_D(output_max, targetvf)              
                errsimilar_max.backward()

                # output_m = similar(noise_mumax.detach(),mumax.detach()) # 后面改进similar，这是跟mu的
                # output_m = output_m.squeeze()
                # errsimilar_m = criterion_D(output_m, targetvr)
                # errsimilar_m.backward()

                output_ = similar(noise_mu.detach(),noise_mumax.detach()) # 后面改进similar，这是跟mu的
                ...
                targetv = Variable(similar_label.fill_(real_label)) # real_label = 1 如果改成fake_label=0.性能爆降
                output = netD_S(fake)
                ...
                fake = net.decoder(latent_zhong) # (2) Update D_S network 只要保留这一行，就可以顺利运行，其他的都可以注释 x_mu
                ...
                #self.mu_max = self.mu_max+mu_lr*(torch.mean(self.mu-latent_max))
                # old_rec1,old_latent1 = net(self.inputs_old_min,get_latent=True)
                # rec_loss_min = criterion(rec_min, inputs_min)
                # rec_loss2 = criterion(old_rec1, self.inputs_old_min) # ? 这里不对
                ...
                # self.inputs_old_min=inputs_line1[index_sorted[:int(latent_min.size(0))]]
                ...
                rec_loss = criterion(old_rec, self.inputs_old_max) # ? 这里不对
    ...
    def test(self, dataset: BaseADDataset, ae_net: BaseNet):
        ...
        with open('spl.txt', 'a') as f:
            ...
            f.write(str('normal_cls is {:.2f}-->'.format(self.normal_cls)))
