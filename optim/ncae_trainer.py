...
class NCAETrainer(BaseTrainer):
    ...
    def train(self, dataset: BaseADDataset, net: BaseNet,d_l, d_s,d_g,g,d_k):
        ...
        for epoch in range(self.n_epochs):
            ...
            for _s, data in enumerate(train_loader):
                ...
                # output_ = similar(noise_mumax.detach(),x_mu.detach()) # 后面改进similar，这是跟mu的
                ...
                fake = net.decoder(latent_zhong) # (2) Update D_S network 只要保留这一行，就可以顺利运行，其他的都可以注释
    ...
    def test(self, dataset: BaseADDataset, ae_net: BaseNet):
        ...
        with open('results_abl.txt', 'a') as f:
    ...
    # def test(self, dataset: BaseADDataset, ae_net: BaseNet,similar):
    #     logger = logging.getLogger()
    ...
    #             rec,latent = ae_net(inputs,get_latent=True)
    #             x_mu=self.mu.repeat(int(latent.size(0)),1)
    #             output_min = similar(latent.detach(),x_mu.detach()) # 后面改进similar，这是跟mu的
    #             #output_min = output_min.squeeze()
    #             scores = torch.mean(output_min, dim=tuple(range(1, output_min.dim())))
